...
public class ExplosiveSpawner : MonoBehaviour
{
    ...
    private void SpawnBall()
    {
        ...
        if (ball != null)
        {
            float randomX = Random.Range(transform.position.x - _radius, transform.position.x + _radius);
            float randomZ = Random.Range(transform.position.z - _radius, transform.position.z + _radius);
